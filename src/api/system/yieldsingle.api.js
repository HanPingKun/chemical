...
// 产率预测任务的基础URL
const YIELD_SINGLE_BASE_URL = "/yield-task/predict";
...
const YieldSingleAPI = {
  /**
   * 提交单个产率预测任务
   * @param {FormData} formData - 包含输入类型和SMILES信息的表单数据
   */
  createSingleTask(formData) {
    return request({
      url: `${YIELD_SINGLE_BASE_URL}/create-single`,
      method: "post",
  ...
  getPage(queryParams) {
    return request({
      url: `${YIELD_SINGLE_BASE_URL}/query-page`,
      method: "get",
      params: queryParams,
      headers: {
        USER_ID: fixedUserInfo.userId,
  ...
  deleteByIds(ids) {
    return request({
      url: `${YIELD_SINGLE_BASE_URL}/delete`,
      method: "delete",
